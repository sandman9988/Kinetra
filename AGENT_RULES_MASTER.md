...
> **Canonical plan:** [`docs/MENU_RESTRUCTURE_PLAN.md`](docs/MENU_RESTRUCTURE_PLAN.md)
>
...
> **Canonical plan:** [`docs/DRY_REMEDIATION_PLAN.md`](docs/DRY_REMEDIATION_PLAN.md)
>
...
- ✅ Check `docs/DRY_REMEDIATION_PLAN.md` before touching any of the open items
...
See `docs/DRY_REMEDIATION_PLAN.md` for the full status table and per-item notes.
...
Always use Python `len()` for content lines in architecture diagrams — they
contain only ASCII characters.  Reserve `wcwidth.wcswidth()` for the splash
block which contains box-drawing and block characters (see §30.12.5).
...
#### 30.12.5 Splash / logo block — `_splash()` in `main()`

The startup splash uses a **separate** inner width (`_INNER = 75`) and
**double-line** borders (`╔ ═ ╗ / ╚ ═ ╝ / ║`) because it contains the block
logo art which uses multi-column Unicode characters (full-block `█`, box-drawing
`╗`, etc.) that `len()` cannot measure correctly.

**Must use `wcwidth.wcswidth()` to measure display width — never `len()`:**

```python
try:
    from wcwidth import wcswidth as _wcswidth
except ImportError:  # noqa: F401
    _wcswidth = len  # type: ignore[assignment]   ← CI / pipe fallback
...
The `_splash()` function lives inside `main()` so it has access to `_wcswidth`
without polluting module scope.

**Hard rules for the splash block:**
- ❌ **Never** replace `_splash()` with a static triple-quoted string — it will
  misalign on terminals where block/box characters render as 2 columns
- ❌ **Never** use `len()` to pad splash rows — always `wcswidth()`
- ✅ `_INNER = 75` is the canonical splash inner width — do not change it without
  re-measuring every logo line with `wcswidth`
- ✅ The `wcwidth` import must be inside the function with an `except ImportError`
  fallback — `wcwidth` is an optional dependency
...
- ✅ All content lines must be exactly the same width — verify with `len()`

Because these boxes contain only ASCII content, **`len()` is sufficient** — no
`wcwidth` needed.
