...
# ============================================================================
# High-Performance Rolling Percentile (Numba-accelerated)
# ============================================================================


@jit(nopython=True, cache=True, parallel=False)
def _rolling_percentile_core(arr: np.ndarray, window: int) -> np.ndarray:
    """
    Numba-accelerated rolling percentile calculation.

    For each position i, calculates what percentile the current value is
    within the last 'window' values (0.0 to 1.0).

    ~100x faster than pandas .apply() with Python function.
    """
    n = len(arr)
    ...
    for i in range(window - 1, n):
        ...
        # Count values <= current (handling NaN)
        count = 0
        valid = 0
        ...
        if valid > 0:
            ...
            result[i] = 0.5  # Default for no valid data

    return result


@jit(nopython=True, cache=True, parallel=True)
def _rolling_percentile_parallel(arr: np.ndarray, window: int) -> np.ndarray:
    """
    Parallel Numba-accelerated rolling percentile for large arrays.

    Uses parallel processing for arrays > 10,000 elements.
    """
    n = len(arr)
    result = np.empty(n, dtype=np.float64)
...
def rolling_percentile_vectorized(arr: np.ndarray, window: int) -> np.ndarray:
    """
    High-performance rolling percentile calculation.

    Automatically selects between sequential and parallel implementations
    based on array size.

    Args:
        arr: Input array (1D numpy array or convertible)
        window: Rolling window size

    Returns:
        Array of percentile values (0.0 to 1.0)

    Performance:
        - ~100x faster than pandas .apply() for sequential
        - ~200-500x faster with parallel for large arrays

    Example:
        >>> arr = np.random.randn(10000)
        >>> result = rolling_percentile_vectorized(arr, 100)
    """
    ...
    if len(arr) > 10000:
        return _rolling_percentile_parallel(arr, window)
    else:
        return _rolling_percentile_core(arr, window)


# ============================================================================
# Additional High-Performance Functions
# ============================================================================


@jit(nopython=True, cache=True)
def rolling_zscore_fast(arr: np.ndarray, window: int) -> np.ndarray:
    """Fast rolling z-score calculation using Numba."""
    n = len(arr)
    result = np.empty(n, dtype=np.float64)
    result[:] = np.nan
    ...
    for i in range(window - 1, n):
        ...
        for j in range(len(window_data)):
            if not np.isnan(window_data[j]):
                diff = window_data[j] - mean
                sum_sq += diff * diff

        std = np.sqrt(sum_sq / (valid_count - 1))
...
@jit(nopython=True, cache=True)
def ewma_fast(arr: np.ndarray, alpha: float) -> np.ndarray:
...
class TickBuffer:
    ...
    def append_tick(
        ...
        self._buffer[self._head] = (np.datetime64(timestamp, "us"), bid, ask, last, volume, spread)
        self._head = (self._head + 1) % self.capacity
        self._count = min(self._count + 1, self.capacity)
    ...
    def _make_key(self, func: Callable, args: tuple, kwargs: dict) -> str:
        ...
        for arg in args:
            if isinstance(arg, np.ndarray):
                ...
                key_parts.append(
                    str(hash(arg) if hasattr(arg, "__hash__") and arg.__hash__ else id(arg))
                )
                ...
                if now >= info["next_time"]:
                    ...
                    with self._lock:
                        if name in self._callbacks:
                            ...
                            self._total_latencies[name] = (
                                self._total_latencies.get(name, 0) + latency
                            )
...
class OnTickHandler:
    ...
    def on_tick(
        ...
        with self._lock:
            ...
            now = time.time()
            should_process = (
                len(self._pending_ticks) >= self.batch_size
                or (now - self._last_process_time) >= self.min_interval
            )
