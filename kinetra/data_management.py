...
class AtomicFileWriter:
    ...
    @staticmethod
    def write_parquet(df: pd.DataFrame, filepath: Path, **kwargs):
        """Atomically write DataFrame to Parquet."""
        filepath = Path(filepath)
        filepath.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            df.to_parquet(temp_file, **kwargs)
            temp_file.replace(filepath)
            logger.debug(f"Atomically wrote parquet file to {filepath}")
        except Exception as e:
            if temp_file.exists():
                temp_file.unlink()
            raise RuntimeError(f"Failed to write {filepath}: {e}")
...
class MasterDataManager:
    ...
    def _merge_and_update(
        self, filepath: Path, new_df: pd.DataFrame, backup: bool = True
    ) -> Tuple[bool, str]:
...
class TestRunManager:
    ...
    def create_run(
        self, test_suite: str, instruments: List[str], config: Dict, master_data_dir: Path
    ) -> Tuple[str, Path]:
        """
    ...
    def _create_data_snapshot(self, source_dir: Path, target_dir: Path, instruments: List[str]):
...
class DataCoordinator:
    ...
    def prepare_test_run(
        self, test_suite: str, instruments: List[str], config: Dict
    ) -> Tuple[str, Path]:
    ...
    def get_cached_features(
        self, symbol: str, timeframe: str, data_checksum: str
    ...
    def cache_features(
        self, symbol: str, timeframe: str, data_checksum: str, features_df: pd.DataFrame
    ):
