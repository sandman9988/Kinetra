...
class InstrumentRegistry:
    """
    Dynamic registry of instruments by asset class.

    Discovers instruments from actual data files instead of hardcoded lists.
    Falls back to hardcoded defaults if data discovery fails.
    """

    # Fallback defaults if data discovery fails
    ...
    def _get_discovery(cls):
        """Get or create DataDiscovery instance."""
        if cls._discovery is None:
            try:
                # Import here to avoid circular dependency
                sys.path.insert(0, str(Path(__file__).parent))
                from kinetra.data_discovery import DataDiscovery
                cls._discovery = DataDiscovery()
                logger.info("Using dynamic data discovery for instruments")
            except Exception as e:
                logger.warning(f"Failed to initialize data discovery: {e}")
                logger.warning("Falling back to hardcoded instrument list")
    ...
    def get_instruments(cls, asset_class: str) -> List[str]:
        """
        Get instruments for asset class from discovered data.

        Args:
            asset_class: Asset class to query

        Returns:
            List of available symbols for that asset class
        """
        discovery = cls._get_discovery()

        if discovery:
            try:
                # Get symbols from discovered data
                symbols = discovery.get_symbols(asset_class)
                if symbols:
                    logger.debug(f"Discovered {len(symbols)} instruments for {asset_class}")
                    return symbols
            except Exception as e:
                logger.warning(f"Data discovery failed for {asset_class}: {e}")

        # Fallback to hardcoded list
        fallback = list(cls.FALLBACK_INSTRUMENTS.get(asset_class, []))
        logger.debug(f"Using fallback instruments for {asset_class}: {len(fallback)} symbols")
        return fallback

    @classmethod
    @lru_cache(maxsize=1)
    def get_all_instruments(cls) -> List[str]:
        """Get all available instruments across all asset classes."""
        discovery = cls._get_discovery()

        if discovery:
            try:
                # Get all symbols from discovered data
                symbols = discovery.get_symbols()
                if symbols:
                    logger.debug(f"Discovered {len(symbols)} total instruments")
                    return symbols
            except Exception as e:
                logger.warning(f"Data discovery failed: {e}")
    ...
    def get_top_instruments(cls, asset_class: str, n: int = 3) -> List[str]:
        ...
        Uses discovered data to find most common instruments.
        ...
        if discovery:
            try:
                # Use data discovery to get top N by file count
                top = discovery.get_top_symbols(asset_class, n=n, by='files')
                if top:
                    logger.debug(f"Top {n} {asset_class} instruments: {top}")
                    return top
            except Exception as e:
                logger.warning(f"Data discovery failed for top {asset_class}: {e}")

        # Fallback to first N from list
