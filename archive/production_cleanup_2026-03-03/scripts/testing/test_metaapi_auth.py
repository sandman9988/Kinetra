...
def api_token():
    ...
    if not token:
        pytest.skip("METAAPI_TOKEN not set. Run: python scripts/setup_metaapi_credentials.py")
    return token
...
async def test_metaapi_authentication(api_token, account_id):
    ...
    try:
        ...
        print("   Get token from: https://app.metaapi.cloud/")
        return False
    ...
    try:
        ...
        if account_id is None:
            if len(accounts) == 1:
                ...
                print("For now, using the first account...")
                account = accounts[0]
    ...
    print("\n  💡 Note:")
    print("    Data is organized by broker and account to prevent corruption.")
    print("    Each account's data is stored separately under:")
...
if __name__ == "__main__":
    ...
    # Run: python scripts/setup_metaapi_credentials.py to configure
    import os
    API_TOKEN = os.getenv("METAAPI_TOKEN")
    if not API_TOKEN:
        print("❌ METAAPI_TOKEN not set. Run: python scripts/setup_metaapi_credentials.py")
        return
