...
def main():
    ...
    if not loader.instruments:
        ...
        print("   SYMBOL_TIMEFRAME_STARTDATE_ENDDATE.csv")
        return
    ...
    print("   • Quick test: 10 episodes (~2 min)")
    print("   • Standard: 30 episodes (~5 min)")
    print("   • Thorough: 50 episodes (~10 min)")
    ...
    print("   1. LinearQ only (fastest, baseline)")
    print("   2. LinearQ + PPO (on-policy comparison)")
    print("   3. LinearQ + SAC (off-policy comparison)")
    print("   4. All agents (LinearQ + PPO + SAC + TD3)")
    ...
    print("   → All agents will see IDENTICAL instrument sequence")
    print("   → All agents will start at IDENTICAL bar positions")
    ...
    for agent_name, agent in agents.items():
        ...
        try:
            ...
            results_file = persistence.results_dir / "agent_comparison_progress.json"
    ...
    final_results_file = persistence.results_dir / "agent_comparison_FINAL.json"
    ...
    print("\n🔬 Next Steps:")
    print("  1. Review which agent(s) performed best")
    print("  2. If one dominates → use it universally")
    print("  3. If different agents excel → explore specialization")
    print("  4. Test measurement impact per winning agent")
