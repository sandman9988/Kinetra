...
def main():
    ...
    if issues:
        ...
        print("\n💡 Recommendations:")
        print("  1. Fetch missing data: python scripts/download_metaapi.py")
        print("  2. Run data preparation to handle:")
        print("     • Public holidays")
        print("     • Trading hours (forex 24/5, indices market hours)")
        print("     • Missing data interpolation")
        print("     • Timezone alignment")

        response = input("\nContinue with existing data anyway? [y/N]: ").strip().lower()
        if response not in ['y', 'yes']:
            print("\n⚠️  Exploration cancelled")
            print("\nNext steps:")
            print("  1. Fix data issues (download missing data, run data prep)")
