...
def run_backtest(df: pd.DataFrame, strategy_class, cash: float = 100000) -> dict:
    ...
    try:
        ...
        return {
            ...
            'profit_factor': float(stats['Profit Factor']) if not np.isnan(stats['Profit Factor']) else 0,
...
def plot_comparison(results_df: pd.DataFrame, title: str = "Strategy Comparison"):
    ...
    ax1.axvline(x=df['buy_hold'].iloc[0], color='blue', linestyle='--', label=f"Buy&Hold: {df['buy_hold'].iloc[0]:.1f}%")
    ...
    ax2 = axes[0, 1]
    scatter = ax2.scatter(df['return_pct'], df['sharpe'],
                         c=df['max_dd'].abs(), cmap='RdYlGn_r',
                         s=100, alpha=0.7)
    for i, row in df.iterrows():
        ax2.annotate(row['strategy'], (row['return_pct'], row['sharpe']),
    ...
    if len(valid_pf) > 0:
        ax3.scatter(valid_pf['win_rate'], valid_pf['profit_factor'],
                   s=valid_pf['trades']*2, alpha=0.7)
        for i, row in valid_pf.iterrows():
            ax3.annotate(row['strategy'], (row['win_rate'], row['profit_factor']),
                        fontsize=8, ha='center', va='bottom')
...
def plot_equity_curves(df: pd.DataFrame, results_df: pd.DataFrame, title: str = "Equity Curves"):
    ...
    for _, row in results_df.iterrows():
        if row['success'] and 'bt' in row and row['bt'] is not None:
            try:
                stats = row['bt'].run()
...
def main():
    ...
    for filepath in csv_files:
        ...
        try:
            ...
            fig1.savefig(f"results_{filename.replace('.csv', '')}_comparison.png", dpi=150, bbox_inches='tight')
            print(f"  Saved: results_{filename.replace('.csv', '')}_comparison.png")

            fig2 = plot_equity_curves(df, results, f"{filename} - Equity Curves")
            fig2.savefig(f"results_{filename.replace('.csv', '')}_equity.png", dpi=150, bbox_inches='tight')
    ...
    if len(all_results) > 1:
        ...
        avg_perf = combined[combined['success']].groupby('strategy').agg({
            'return_pct': 'mean',
            'sharpe': 'mean',
            'max_dd': 'mean',
            'win_rate': 'mean',
