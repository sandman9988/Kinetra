...
class FatCandleForensics:
    ...
    def analyze_preconditions(
        ...
        for _, fat_candle in fat_candles.iterrows():
            ...
            preconditions['volume_ratio_before'].append(recent_volume.mean() / volume_ma if volume_ma > 0 else 1.0)
    ...
    def compare_to_normal_bars(
        ...
        for _, fat_candle in fat_candles.iterrows():
            ...
            fat_preconditions.append({
                'returns_std': df['close'].pct_change().iloc[lookback_slice].std(),
                'volume_ratio': df['volume'].iloc[lookback_slice].mean() / df['volume'].rolling(50).mean().iloc[idx],
                'energy_mean': physics['energy'].iloc[lookback_slice].mean() if 'energy' in physics.columns else np.nan,
                'damping_mean': physics['damping'].iloc[lookback_slice].mean() if 'damping' in physics.columns else np.nan,
                'reynolds_mean': physics['reynolds'].iloc[lookback_slice].mean() if 'reynolds' in physics.columns else np.nan,
            })

        fat_df = pd.DataFrame(fat_preconditions)
    ...
    for asset_class, instruments in class_samples.items():
        ...
        for instrument in instruments:
            ...
            for _, row in significant.iterrows():
                print(f"    {row['measurement']:20s}: Cohen's d = {row['cohens_d']:6.2f}, p = {row['p_value']:.4f}")
