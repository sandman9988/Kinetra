...
def benchmark_sample_entropy():
    ...
    try:
        from kinetra.performance import check_numba_available, sample_entropy_fast
...
def benchmark_rolling_entropy():
    """Benchmark rolling entropy computation."""
    print_header("4b. ROLLING ENTROPY BENCHMARK")
    ...
    def pandas_entropy(series: pd.Series, win: int, histogram_bins: int) -> pd.Series:
        def entropy_window(window_values: pd.Series) -> float:
            arr = window_values.to_numpy(dtype=float, copy=False)
            arr = arr[np.isfinite(arr)]
            if arr.size < 5:
                return np.nan
            hist, _ = np.histogram(arr, bins=histogram_bins, density=True)
            probabilities = hist / (hist.sum() + 1e-12)
            probabilities = probabilities[probabilities > 0]
            entropy = -np.sum(probabilities * np.log(probabilities))
            return float(entropy / np.log(histogram_bins))

        return series.rolling(win, min_periods=5).apply(entropy_window, raw=False)

    start = time.perf_counter()
    baseline = pandas_entropy(data, window, bins)
    pandas_time = time.perf_counter() - start

    try:
        from kinetra.performance import rolling_entropy_vectorized

        start = time.perf_counter()
        vectorized = rolling_entropy_vectorized(
            data.to_numpy(dtype=float, copy=True),
            window,
            bins,
            min_periods=5,
        )
        ...
        baseline_values = baseline.to_numpy(dtype=float, copy=False)
        finite_mask = np.isfinite(baseline_values) & np.isfinite(vectorized)
        max_delta = float("nan")
        if np.any(finite_mask):
            max_delta = float(
                np.max(np.abs(baseline_values[finite_mask] - vectorized[finite_mask]))
            )

        print(f"\n  Data size: {n}, window: {window}, bins: {bins}")
        print_result("  Pandas .apply()", pandas_time)
        print_result("  Vectorized", opt_time, pandas_time)
        if np.isfinite(max_delta):
            print(f"  Max absolute delta: {max_delta:.3e}")
        else:
            print("  Max absolute delta: NaN (insufficient overlapping data)")
    except ImportError:
        print(f"\n  Data size: {n}, window: {window}, bins: {bins}")
        print_result("  Pandas .apply()", pandas_time)
        print("  Vectorized entropy: Not available (install kinetra.performance)")
...
def benchmark_data_loading():
    ...
    from kinetra.data_utils import clear_format_cache, load_mt5_csv
