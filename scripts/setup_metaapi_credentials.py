...
def get_token_from_user() -> Optional[str]:
    ...
    try:
        ...
        if not token.startswith("eyJ"):
            print("⚠️  Warning: Token doesn't look like a JWT (should start with 'eyJ')")
...
async def validate_token(token: str) -> bool:
    ...
    try:
        ...
        print(f"✅ Token is valid! Found {len(accounts)} account(s)")
        return True

    except ImportError:
        print("⚠️  metaapi-cloud-sdk not installed - skipping validation")
...
async def list_and_select_accounts(token: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        if not accounts:
            print("❌ No accounts found in your MetaAPI account")
...
def select_accounts_interactively(account_list: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    try:
        ...
        labeled_accounts = []
        for acc in selected:
            print(f"\n📋 Configure account: {acc['name']} (ID: {acc['id']})")
...
async def setup_credentials(add_account_only: bool = False):
    ...
    if add_account_only and "METAAPI_TOKEN" in env_vars:
        print("✅ Using existing METAAPI_TOKEN from .env")
        ...
        if not await validate_token(token):
            print("\n❌ Token validation failed - setup cancelled")
            return
    ...
    if not account_list:
        print("\n⚠️  No accounts available to configure")
    ...
    print(f"✅ Default account: {selected_accounts[0]['name']}")
...
def main():
    ...
    try:
        ...
        print("❌ ERROR: metaapi-cloud-sdk package not installed")
        print("   Install with: pip install metaapi-cloud-sdk")
