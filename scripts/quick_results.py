...
def main():
    ...
    for symbol, filename in test_files:
        ...
        for agent in agents:
            ...
            if 'error' not in result:
                print(f"    {agent.name:15} | Return: {result['total_return']:>7.2f}% | "
    ...
    if all_results:
        ...
        print("-" * 70)
        for agent_name, row in summary.iterrows():
            pf = f"{row['profit_factor']:.2f}" if row['profit_factor'] < 100 else "∞"
            print(f"{agent_name:<15} {row['total_return']:>11.2f}% {int(row['trades']):>14} "
                  f"{row['win_rate']:>9.1f}% {row['max_drawdown']:>9.1f}% {pf:>10}")
