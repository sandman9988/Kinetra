...
class TestRunner:
    ...
    def run_basic_tests(self) -> Dict[str, bool]:
        ...
        success, output = run_test("Menu System Tests", [sys.executable, str(test_path)])
    ...
    def run_workflow_tests(self) -> Dict[str, bool]:
        ...
        success, output = run_test("Menu Workflow Tests", [sys.executable, str(test_path)])
    ...
    def run_stress_tests(self, level: str = "standard") -> Dict[str, bool]:
        ...
        success, output = run_test(
            f"System Stress Test ({level})", [sys.executable, str(test_path)] + args
        )
    ...
    def run_performance_profiling(self) -> Dict[str, bool]:
        ...
        success, output = run_test(
            "Performance Profiling", [sys.executable, str(test_path), "--full", "--save"]
        )
...
def main():
    ...
    parser.add_argument(
        "--full", action="store_true", help="Run all tests including heavy stress test"
    )

    parser.add_argument(
        "--with-profiling", action="store_true", help="Include performance profiling (adds ~30s)"
    )
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="Enable live MT5 data fetch/stream (uses ACCESS_TOKEN/ACCOUNT_ID env vars)",
    )

    args = parser.parse_args()

    if args.live:
        print("\n🔴 Live Mode: Fetching BTCUSD H1 via MT5...")
        cmd = [
            sys.executable,
        ...
        if result.returncode == 0:
            os.environ["KINETRA_USE_LIVE_DATA"] = "1"
            print("✅ Live data fetched and saved. Tests will use it.")
        else:
            print("⚠️ Live fetch failed (fallback to synthetic): " + result.stderr[:200])
